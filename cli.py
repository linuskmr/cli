...
class Cli:
    """CLI parser inspired by https://github.com/tiangolo/typer"""
    ...
    def create_main_parser(self):
        ...
        if self._main_function:
            Cli.add_args(parser=parser, function=self._main_function)
        return parser
    ...
    def create_subparsers(self, parser: ArgumentParser):
        ...
        subparsers = parser.add_subparsers(dest=Cli.SUBCOMMAND_DEST)
        for function_name, function in self._subcommand_functions.items():
            ...
            Cli.add_args(parser=subparser, function=function)
